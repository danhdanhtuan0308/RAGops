...
def build_eess_query(categories: Optional[List[str]] = None) -> str:
    cats = categories or EESS_CATEGORIES
...
def fetch_eess(
    ...
    """
    Harvest arXiv Electrical Engineering & Systems Science (eess.*) papers from 2023-01-01 up to 'to_date' (default: 2025-03-01).
    ...
    logging.info(f"Harvesting {len(windows)} monthly windows from {start} to {end} (eess categories).")
    ...
    if out_parquet is None:
        out_parquet = f"arxiv_eess_{start.strftime('%Y%m%d')}_{end.strftime('%Y%m%d')}.parquet"
    gcs_uri = f"gs://research-paper857/research/eess_{start.strftime('%Y%m%d')}_{end.strftime('%Y%m%d')}.parquet"
...
if __name__ == "__main__":
    fetch_eess(page_size=200, delay=3.0)
