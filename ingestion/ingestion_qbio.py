...
# Quantitative Biology categories (q-bio + subcategories)
BIO_CATEGORIES = [
    "q-bio",         # top-level catch-all
...
def build_bio_query(categories: Optional[List[str]] = None) -> str:
    cats = categories or BIO_CATEGORIES
...
def fetch_bio(
    ...
    logging.info(f"Harvesting {len(windows)} monthly windows from {start} to {end} (q-bio categories).")
...
if __name__ == "__main__":
    fetch_bio(page_size=200, delay=3.0)
