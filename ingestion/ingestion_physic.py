...
# Physics-related categories (physics.* + major physics top-levels)
PHYSICS_CATEGORIES = [
    # physics.*
    ...
    # high-energy, gravity, math-phys, nuclear, quantum
...
def build_physics_query(categories: Optional[List[str]] = None) -> str:
    cats = categories or PHYSICS_CATEGORIES
...
def fetch_physics(
    ...
    """
    Harvest physics-related arXiv papers from 2023-01-01 up to 'to_date' (default: 2025-03-01).
    Saves to Parquet (GCS) and returns a pandas DataFrame.
    """
    start = datetime(2024, 7, 1)
    end_dt = datetime(2025, 3, 1) if to_date is None else to_date
    ...
    logging.info(f"Harvesting {len(windows)} monthly windows from {start} to {end} (physics categories).")
    ...
    if out_parquet is None:
        out_parquet = f"arxiv_physics_{start.strftime('%Y%m%d')}_{end.strftime('%Y%m%d')}.parquet"
    gcs_uri = f"gs://research-paper857/research/physics_{start.strftime('%Y%m%d')}_{end.strftime('%Y%m%d')}.parquet"

    try:
        # Requires pyarrow or fastparquet and gcsfs installed; using ADC via storage_options
        df.to_parquet(
            gcs_uri,
            index=False,
...
if __name__ == "__main__":
    fetch_physics(page_size=200, delay=3.0)
