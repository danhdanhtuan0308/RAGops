...
@dag(
    dag_id="econ_ingestion_monthly",
    description="Fetch arXiv Economics papers monthly and store in GCS",
    schedule="0 0 1 * *",                # monthly, 00:00 on the 1st
    start_date=datetime(2025, 10, 1),     # bump to 2025-10-01 when ready
    catchup=False,
    default_args=default_args,
    tags=["arxiv", "economics", "ingestion"],
)
def econ_ingestion_monthly():
    start = EmptyOperator(task_id="start")
    end = EmptyOperator(task_id="end")

    @task(task_id="fetch_for_interval")
    def run_ingestion_for_window():
        logging.info("Starting ECON ingestion task")
        try:
            ...
            # Robust import from your ingestion_econ module
            try:
                from ingestion.ingestion_econ import build_econ_query, fetch_window
                logging.info("Imported ingestion.ingestion_econ functions")
            except Exception as e1:
                logging.warning(f"Package function import failed ({e1}); trying package module")
                try:
                    from ingestion import ingestion_econ as _mod
                    build_econ_query = getattr(_mod, "build_econ_query")
                    ...
                    import importlib
                    _mod2 = importlib.import_module("ingestion_econ")
                    build_econ_query = getattr(_mod2, "build_econ_query")
                    fetch_window = getattr(_mod2, "fetch_window")
                    logging.info("Imported top-level ingestion_econ module")
            ...
            out_csv = os.path.join(
                out_dir,
                f"arxiv_econ_{wstart:%Y%m%d}_{wend_exclusive:%Y%m%d}.csv",
            )
            df.to_csv(out_csv, index=False)
            logging.info(f"Saved CSV with {len(df)} rows to {out_csv}")

            # GCS Parquet: dated + stable for BigQuery
            gcs_prefix = f"gs://{GCS_BUCKET}/{GCS_PREFIX}"
            dated_uri  = f"{gcs_prefix}/econ_{wstart:%Y%m%d}_{wend_exclusive:%Y%m%d}.parquet"
            stable_uri = f"{gcs_prefix}/econ_latest.parquet"
