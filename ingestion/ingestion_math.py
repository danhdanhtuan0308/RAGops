...
def build_math_query(categories: Optional[List[str]] = None) -> str:
...
def fetch_math(
    ...
    """
    Harvest arXiv Mathematics (math.*) papers from 2023-01-01 up to 'to_date' (default: 2025-03-01).
    ...
    logging.info(f"Harvesting {len(windows)} monthly windows from {start} to {end} (math categories).")
    ...
    if out_parquet is None:
        out_parquet = f"arxiv_math_{start.strftime('%Y%m%d')}_{end.strftime('%Y%m%d')}.parquet"
    gcs_uri = f"gs://research-paper857/research/math_{start.strftime('%Y%m%d')}_{end.strftime('%Y%m%d')}.parquet"
...
if __name__ == "__main__":
    fetch_math(page_size=200, delay=3.0)
