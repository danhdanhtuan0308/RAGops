...
# Quantitative Finance categories (q-fin + subcategories)
QFIN_CATEGORIES = [
    "q-fin",      # top-level catch-all
    "q-fin.CP",   # Computational Finance
    "q-fin.EC",   # Economics (q-fin cross-area)
...
def build_qfin_query(categories: Optional[List[str]] = None) -> str:
...
def fetch_quant_finance(
    ...
    logging.info(f"Harvesting {len(windows)} monthly windows from {start} to {end} (q-fin categories).")
    ...
    if out_parquet is None:
        out_parquet = f"arxiv_qfin_{start.strftime('%Y%m%d')}_{end.strftime('%Y%m%d')}.parquet"
    gcs_uri = f"gs://research-paper857/research/qfin_{start.strftime('%Y%m%d')}_{end.strftime('%Y%m%d')}.parquet"
...
if __name__ == "__main__":
    fetch_quant_finance(page_size=200, delay=3.0)
