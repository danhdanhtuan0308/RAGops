...
# Add ingestion module to Python path (so we can import your ingestion_qfin.py)
...
def qfin_ingestion_monthly():
    ...
    def run_ingestion_for_window():
        ...
        try:
            ...
            # Robust import from your ingestion_qfin module (no codebase changes)
            try:
                from ingestion.ingestion_qfin import build_qfin_query, fetch_window
                logging.info("Imported ingestion.ingestion_qfin functions")
            except Exception as e1:
                logging.warning(f"Package function import failed ({e1}); trying package module")
                try:
                    from ingestion import ingestion_qfin as _mod
                    build_qfin_query = getattr(_mod, "build_qfin_query")
                    ...
                    import importlib
                    _mod2 = importlib.import_module("ingestion_qfin")  # expects ingestion_qfin.py on PYTHONPATH
                    build_qfin_query = getattr(_mod2, "build_qfin_query")
                    fetch_window = getattr(_mod2, "fetch_window")
                    logging.info("Imported top-level ingestion_qfin module")
            ...
            out_csv = os.path.join(
                out_dir,
                f"arxiv_qfin_{wstart:%Y%m%d}_{wend_exclusive:%Y%m%d}.csv",
            )
            ...
            dated_uri  = f"{gcs_prefix}/qfin_{wstart:%Y%m%d}_{wend_exclusive:%Y%m%d}.parquet"
            stable_uri = f"{gcs_prefix}/qfin_latest.parquet"
