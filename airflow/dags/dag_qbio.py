...
# ---- Config (edit bucket/prefix if needed) ----
...
@dag(
    dag_id="qbio_ingestion_monthly",
    description="Fetch arXiv Quantitative Biology (q-bio) papers monthly and store in GCS",
    schedule="0 0 1 * *",
    start_date=datetime(2025, 10, 1),
    catchup=False,
    default_args=default_args,
    tags=["arxiv", "q-bio", "ingestion"],
)
def qbio_ingestion_monthly():
    start = EmptyOperator(task_id="start")
    end = EmptyOperator(task_id="end")

    @task(task_id="fetch_for_interval")
    def run_ingestion_for_window():
        logging.info("Starting q-bio ingestion task")
        try:
            ...
            # Import from your q-bio ingestion module (no codebase changes)
            try:
                from ingestion.ingestion_qbio import build_bio_query, fetch_window
                logging.info("Imported ingestion.ingestion_qbio functions")
            except Exception as e1:
                logging.warning(f"Package function import failed ({e1}); trying package module")
                try:
                    from ingestion import ingestion_qbio as _mod
                    build_bio_query = getattr(_mod, "build_bio_query")
                    ...
                    import importlib
                    _mod2 = importlib.import_module("ingestion_qbio")
                    build_bio_query = getattr(_mod2, "build_bio_query")
                    fetch_window = getattr(_mod2, "fetch_window")
                    logging.info("Imported top-level ingestion_qbio module")
            ...
            out_csv = os.path.join(
                out_dir,
                f"arxiv_qbio_{wstart:%Y%m%d}_{wend_exclusive:%Y%m%d}.csv",
            )
            df.to_csv(out_csv, index=False)
            logging.info(f"Saved CSV with {len(df)} rows to {out_csv}")

            # GCS Parquet: dated + stable (point BigQuery at the stable one)
            gcs_prefix = f"gs://{GCS_BUCKET}/{GCS_PREFIX}"
            dated_uri  = f"{gcs_prefix}/qbio_{wstart:%Y%m%d}_{wend_exclusive:%Y%m%d}.parquet"
            stable_uri = f"{gcs_prefix}/qbio_latest.parquet"
