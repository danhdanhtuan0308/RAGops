...
#Math config (edit bucket/prefix here) 
...
@dag(
    dag_id="math_ingestion_monthly",
    description="Fetch arXiv Math (math.*) papers monthly and store in GCS",
    schedule="0 0 1 * *",                # monthly at 00:00 on the 1st
    start_date=datetime(2025, 10, 1),     # bump to 2025-10-01 when you’re ready
    catchup=False,
    default_args=default_args,
    tags=["arxiv", "math", "ingestion"],
)
def math_ingestion_monthly():
    start = EmptyOperator(task_id="start")
    end = EmptyOperator(task_id="end")

    @task(task_id="fetch_for_interval")
    def run_ingestion_for_window():
        logging.info("Starting Math ingestion task")
        try:
            ...
            # Robust import from your ingestion_math module (no codebase changes)
            try:
                from ingestion.ingestion_math import build_math_query, fetch_window
                logging.info("Imported ingestion.ingestion_math functions")
            except Exception as e1:
                logging.warning(f"Package function import failed ({e1}); trying package module")
                try:
                    from ingestion import ingestion_math as _mod
                    build_math_query = getattr(_mod, "build_math_query")
                    ...
                    import importlib
                    _mod2 = importlib.import_module("ingestion_math")
                    build_math_query = getattr(_mod2, "build_math_query")
                    fetch_window = getattr(_mod2, "fetch_window")
                    logging.info("Imported top-level ingestion_math module")

            # Fetch using your ingestion primitives
            q_base = build_math_query()
            ...
            out_csv = os.path.join(
                out_dir,
                f"arxiv_math_{wstart:%Y%m%d}_{wend_exclusive:%Y%m%d}.csv",
            )
            ...
            dated_uri  = f"{gcs_prefix}/math_{wstart:%Y%m%d}_{wend_exclusive:%Y%m%d}.parquet"
            stable_uri = f"{gcs_prefix}/math_latest.parquet"
