...
def build_econ_query(categories: Optional[List[str]] = None) -> str:
    cats = categories or ECON_CATEGORIES
    ...
    while True:
        ...
        while True:
            try:
                ...
                if http_retries > max_http_retries:
                    logging.info("[%s] HTTP %s at start=%s; giving up this window page.",
                                 f"{wstart:%Y-%m}", r.status_code, start_index)
                    return results
                backoff = 2 ** http_retries
                logging.info("[%s] HTTP %s at start=%s; retry %s in %ss.",
                             f"{wstart:%Y-%m}", r.status_code, start_index, http_retries, backoff)
                time.sleep(backoff)
            except requests.RequestException as e:
                http_retries += 1
                if http_retries > max_http_retries:
                    logging.info("[%s] Request error %r at start=%s; giving up this window page.",
                                 f"{wstart:%Y-%m}", e, start_index)
                    return results
                backoff = 2 ** http_retries
                logging.info("[%s] Request error %r; retry %s in %ss.",
                             f"{wstart:%Y-%m}", e, http_retries, backoff)
                time.sleep(backoff)
        ...
        if not entries:
            if empty_retries < max_empty_retries:
                empty_retries += 1
                logging.info("[%s] Empty page at start=%s; retry %s after short backoff.",
                             f"{wstart:%Y-%m}", start_index, empty_retries)
                time.sleep(2 + empty_retries)
                continue
            logging.info("[%s] No more entries at start=%s.", f"{wstart:%Y-%m}", start_index)
            break
        ...
        logging.info("[%s] got %d (start=%s)", f"{wstart:%Y-%m}", len(entries), start_index)
...
def fetch_econ(
    ...
    """
    Harvest arXiv Economics (econ.*) papers from 2024-07-01 up to 'to_date' (default: 2025-03-01).
    ...
    q_base = build_econ_query()
    all_rows: List[dict] = []

    windows = month_windows(start, end)
    logging.info("Harvesting %d monthly windows from %s to %s (econ categories).", len(windows), start, end)
    ...
    for wstart, wend in windows:
        ...
        logging.info("[%s] total accumulated this month: %d", f"{wstart:%Y-%m}", len(rows))
    ...
    if out_parquet is None:
        out_parquet = f"arxiv_econ_{start.strftime('%Y%m%d')}_{end.strftime('%Y%m%d')}.parquet"
    gcs_uri = f"gs://research-paper857/research/econ_{start.strftime('%Y%m%d')}_{end.strftime('%Y%m%d')}.parquet"
...
if __name__ == "__main__":
    fetch_econ(page_size=200, delay=3.0)
